                ...
                while (!taskQueue.isEmpty()) {
                    try {
                        ...
                        if (sipConfig.isAlarm()) {
                            if (sipConfig.isCapture()) {
                                // 获取截图 （不需要持久化，存储在指定的位置，根据设备编号和报警时间查找即可）
                                logger.info("[收到报警通知-报警抓图获取]准备获取截图");
                                String fileName = "alarm/"+deviceAlarm.getDeviceId() + "_" + channelId + "_" + deviceAlarm.getAlarmTime() + ".jpg";
                                logger.info("[收到报警通知-报警抓图获取]截图的文件路径");
                                playService.getSnap(deviceAlarm.getDeviceId(), channelId, fileName, (code, msg, data) -> {
                                    logger.info("[收到报警通知-报警抓图获取]code：{}", code);
                                    logger.info("[收到报警通知-报警抓图获取]msg：{}", msg);
                                    logger.info("[收到报警通知-报警抓图获取]data：{}", data);
                                    if (code == InviteErrorCode.SUCCESS.getCode()) {
                                        logger.info("[收到报警通知-报警抓图获取]完整截图文件路径：{}", data);
                                    }else {
                                        logger.info("[收到报警通知-报警抓图获取]生成截图文件错误");
                                    }
                                });
                            }
